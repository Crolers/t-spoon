...
import crolers.tgstream.tgraph.durability.ProxyWALServer;
...
public class NetUtils {
    ...
    // ensure that singleton servers' base port is at least at the specified distance
    ...
    public static <T extends AbstractServer> T getServer(int startPort, int endPort, T server) throws IOException {
        ...
        //启动 AbstractServer 线程
        new Thread(server).start();
        return server;
    }

    //every type of server has a range of ports to be listened
