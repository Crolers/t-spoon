 ...
 * I.e., tasks are executed in FIFO order with respect to the keys provided on #run.
...
public class KeyLevelTaskExecutor<R> {
    ...
    public synchronized void run(long taskID, String key, Supplier<R> task)
        ...
        //Map.computeIfAbsent() 在 key 对应的 value 不存在时，使用 mappingFunction 计算一个新的 value 并将其存入 map 中,返回最近的 value
        ListIterator<Task> iterator = queues.computeIfAbsent(key, k -> {
            freeKeys.add(k);
            return new LinkedList<>();
        }).listIterator();

        //find insert position of the actualTask
