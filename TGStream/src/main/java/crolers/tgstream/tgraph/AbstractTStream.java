...
import crolers.tgstream.tgraph.functions.*;
import crolers.tgstream.tgraph.query.*;
import crolers.tgstream.tgraph.state.*;
import crolers.tgstream.tgraph.twopc.OpenOperator;
...
public abstract class AbstractTStream<T> implements TStream<T> {
    ...
    protected static <T> OpenOutputs<T> open(
        ...
        if (transactionEnvironment.getIsolationLevel() == IsolationLevel.PL4) {
            ...
            openParallelism = 1;
        } else if (transactionEnvironment.getOpenTransactionParallelism() <= 0) {
            // the open operator has parallelism equal to sources
            openParallelism = dataStream.getParallelism();
        } else {
            // the tEnv imposes a parallelism, apply it
            openParallelism = transactionEnvironment.getOpenTransactionParallelism();
        }
        ...
        if (inputQueryStream != null) {
            //根据 inputQueryStream 中每个 query 的 nameSpace，将 query 分到多个流中
    ...
    public <V> StateStream<T> state(
        ...
        DataStream<NoConsensusOperation> wrappedSpus = spus
                ...
                //一个 Slot Sharing Group 中的算子将会被部署在同一个 TaskManager 中，它们可以共享相同的资源，例如 CPU、内存等，
                //并且可以充分利用 TaskManager 的资源，从而提高整个任务的并发度和运行效率。
